# app/services/geodata/bathymetry.py
from __future__ import annotations
from dataclasses import dataclass
from pathlib import Path
from typing import Iterable, List, Tuple, Optional, Dict, Any
import io, json, math, tempfile, requests
import numpy as np
import rasterio
...
EMODNET_COVERAGE = "emodnet:mean"  # średnia głębokość
...
def _bbox_wgs84_from_local_wkt(water_wkt: str, epsg_local: int, pad_m: float = 0.0) -> Tuple[float, float, float, float]:
    """BBOX w WGS84 z geometrii w lokalnym CRS (z opcjonalnym poszerzeniem pad_m)."""
...
def fetch_wcs_geotiff(req: WcsRequest, out_path: Path) -> Path:
    """Ściąga GeoTIFF batymetrii z EMODnet WCS (cache do pliku)."""
    ...
    params = {
        ...
        # Uwaga: część serwerów oczekuje BBOX=ymin,xmin,ymax,xmax – EMODnet przyjmuje klasyczne xmin,ymin,xmax,maxy.
...
def shallow_mask_from_tif(tif_path: Path, epsg_target: int, threshold_m: float) -> MultiPolygon | Polygon | None:
    """
    Z GeoTIFF tworzy poligon mielizn (depth < threshold_m) w EPSG=epsg_target.
    EMODnet 'mean' zwraca głębokości dodatnie (mety) – jeśli trafisz ujemne, weź abs().
    """
    with rasterio.open(tif_path) as ds:
        ...
        # lekkie wygładzenie i odszumianie
        shp_local = shp_local.buffer(10).buffer(-10)
        return shp_local

def contours_geojson_from_tif(tif_path: Path, levels: Iterable[float]) -> Dict[str, Any]:
    """
    Generuje GeoJSON z izobatami (linie) w CRS rastra (WGS84).
    Robimy to poprzez 'marching squares' rasterio+numpy -> shapes na przekroju progowym.
    Prostota > idealne odwzorowanie (wystarczające do mapy podglądowej).
    """
    features = []
    with rasterio.open(tif_path) as ds:
        arr = ds.read(1).astype("float32")
        ...
        for lvl in sorted(levels):
            # maska: bliskie poziomu (okno tolerancji, np. +/- 0.15 m)
            tol = max(0.15, 0.01 * lvl)
            band = np.where(np.isnan(arr), 0, ((arr >= (lvl - tol)) & (arr <= (lvl + tol))).astype("uint8"))
            # polygonize cienkich pasów i wyciąganie linii środkowej (upraszczamy: zewnętrzna granica ≈ izobata)
            for geom, val in rio_shapes(band, mask=band.astype(bool), transform=ds.transform):
                if int(val) != 1:
                    continue
                poly = shape(geom)
                # zamień granicę poligonu na MultiLineString (exterior + interiors)
...
def label_points_along_lines(fc: Dict[str, Any], step_m: float = 1500.0) -> Dict[str, Any]:
    """Generuje proste punkty opisowe (etykiety) co ~step_m wzdłuż linii izobat (do wyświetlenia liczby głębokości)."""
