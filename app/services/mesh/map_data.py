from __future__ import annotations
import json
import numpy as np
from shapely import wkt
from shapely.ops import transform as shp_transform
from pyproj import Transformer
from sqlalchemy.ext.asyncio import AsyncSession
from fastapi.responses import JSONResponse

from app.services.db.services import MeshedAreaService, RoutePointService, RouteService

async def build_map_geojson(session: AsyncSession, mesh_id):
    mesh_svc = MeshedAreaService(session)
    rpoint_svc = RoutePointService(session)
    route_svc = RouteService(session)

    meshed = await mesh_svc.get_entity_by_id(mesh_id, allow_none=False)

    nodes = np.asarray(json.loads(meshed.nodes_json))
    tris = np.asarray(json.loads(meshed.triangles_json), dtype=int)
    water = wkt.loads(meshed.water_wkt)
    route = wkt.loads(meshed.route_wkt)

    transformer = Transformer.from_crs(meshed.crs_epsg, 4326, always_xy=True)
    features = []

    if len(nodes) and len(tris):
        lons, lats = transformer.transform(nodes[:, 0], nodes[:, 1])
        edges = set()
        for t in tris:
            i, j, k = int(t[0]), int(t[1]), int(t[2])
            for a, b in ((i, j), (j, k), (k, i)):
                if a == b: continue
                edges.add((a, b) if a < b else (b, a))
        edges = list(edges)[:30000]  # safety cap
        lines = [ [[float(lons[a]), float(lats[a])], [float(lons[b]), float(lats[b])]] for a,b in edges ]
        features.append({"type":"Feature","properties":{"type":"mesh_wire","edges":len(lines)},
                         "geometry":{"type":"MultiLineString","coordinates":lines}})

    # --- korytarz / trasa ---
    water_wgs = shp_transform(transformer.transform, water)
    features.append({"type":"Feature","properties":{"type":"water_corridor"},"geometry":water_wgs.__geo_interface__})

    route_wgs = shp_transform(transformer.transform, route)
    features.append({"type":"Feature","properties":{"type":"route"},"geometry":route_wgs.__geo_interface__})

    pts_raw = await rpoint_svc.get_all_entities(filters={"route_id": meshed.route_id}, page=1, limit=10000)
    if isinstance(pts_raw, dict) and "results" in pts_raw:
        pts_raw = pts_raw["results"]

    control_points = []
    for p in (pts_raw or []):
        x = getattr(p, "x", None) if not isinstance(p, dict) else p.get("x")
        y = getattr(p, "y", None) if not isinstance(p, dict) else p.get("y")
        idx = getattr(p, "seq_idx", None) if not isinstance(p, dict) else p.get("seq_idx")
        if x is not None and y is not None:
            control_points.append((int(idx or len(control_points)), float(x), float(y)))

    if not control_points:
        route_obj = await route_svc.get_entity_by_id(meshed.route_id, allow_none=False)
        ctrl_json = getattr(route_obj, "control_points", None) or getattr(route_obj, "control_points_json", None)
        if ctrl_json:
            try:
                coords = json.loads(ctrl_json)
                for i, (lon, lat) in enumerate(coords):
                    control_points.append((i, float(lon), float(lat)))
            except Exception:
                pass

    for idx, lon, lat in sorted(control_points, key=lambda r: r[0]):
        features.append({"type":"Feature","properties":{"type":"control_point","seq_idx":idx},
                         "geometry":{"type":"Point","coordinates":[lon, lat]}})

    return JSONResponse({"type":"FeatureCollection","features":features})
